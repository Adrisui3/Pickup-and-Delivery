...
def general_adaptative_metaheuristic(init, init_cost, max_iter, update, beta, problem):
    incumbent = init
    best_solution = init
    cheapest_cost = init_cost
    ...
    for i in range(max_iter):
        ...
        feasible, _ = feasibility_check(current, problem)
        if feasible:
            if tuple(current) not in visited_solutions:
                scores[current_op] += 1
                visited_solutions.add(tuple(current))
            ...
            if delta < 0:
                ...
                if current_cost < cheapest_cost:
                    ...
                    cheapest_cost = current_cost
                    scores[current_op] += 4
            elif current_cost < cheapest_cost + 0.2*((max_iter - i)/max_iter)*cheapest_cost:
                incumbent = current
    ...
    best_solution, cheapest_cost = local_search(best_solution, cheapest_cost, 1500, 1/3, 1/3, 1/3, problem)
